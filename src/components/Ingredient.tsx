...
const Ingredient: React.FC<props> = ({ ingredients }) => {
    ...
    const colors = [
        '#C3E8C1', '#C7F5FE', '#F0E0F8', '#E9FCD2',
        '#FFD5DD', '#FFF2B2', '#F5C9A6',
    ];

    return (
        <View>
            {ingredients && <View style={styles.box}>
...

                {
                    ...
                    >
                        ...
                        >
                            {ingredients?.slice(0, 10).map((item: string, index: number) => {
                                const randomColor = colors[Math.floor(Math.random() * colors.length)];
                                return (
                                    <View key={`${item}-${index}`} style={[styles.ingredientbox, { backgroundColor: randomColor }]}>
